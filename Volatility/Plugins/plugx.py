# PlugX RAT detection and analysis for Volatility 2.6
#
# Version 1.3
#
# Author: Fabien Perigaud <fabien.perigaud@cassidian.com>
# Modified: Stian Svendsen
...
class PlugXConfig(PlugXScan):
	...
	#Config blob and size is taken in (0x27a = 634 bytes)
    def parse_config(self, cfg_blob, cfg_sz, outfd):
        if cfg_sz in (0xae4, 0xbe4, 0x150c, 0x1510, 0x170c, 0x1b18, 0x1d18, 0x2540, 0x254c, 0x2d58, 0x36a4, 0x4ea4,0x27a):
            if cfg_sz == 0x1510:
                cfg_blob = cfg_blob[12:]
	    elif cfg_sz == 0x27a:
		cfg_blob = cfg_blob[16:]
            ...
            # Flags
            if cfg_sz in (0xae4, 0xbe4,0x27a):
                desc = "<L"
            elif cfg_sz in (0x36a4, 0x4ea4):
                desc = "<10L"
            else:
            ...
            str_sz = 0x80 if cfg_sz in (0xae4, 0xbe4) else 0x200
    ...
    def render_text(self, outfd, data):
        ...
        for task, start in data:
            if (task, start) not in found:
                ...
                #print repr(config_blob) #Print out the config blob for test
		print config_blob.encode('hex')
		print repr(config_addr) # print config
		print repr(config_size) #Print config size 634 bytes is sent

		#Print out process with pid and config size
		outfd.write("Process: %s (%d)\n\n" % (task.ImageFileName, task.UniqueProcessId))
                outfd.write("PlugX Config (0x%04x bytes):\n" % config_size)

		#sends the config blob and size up to parse_config method at the top
